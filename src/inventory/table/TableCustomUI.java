...
public class TableCustomUI extends JTable {

    public TableCustomUI() {
        init();
    }

    public TableCustomUI(DefaultTableModel model) {
        super(model);
        init();
    }
    ...
    public Component prepareRenderer(TableCellRenderer tcr, int i, int i1) {
        ...
        if (getValueAt(i, i1) != null) {
            if (getValueAt(i, i1) instanceof DefaultTableModel) {
                ...
                JTable tbl = new TableCustomUI(model);
                tbl.setBackground(com.getBackground());
                autoRowHeight(tbl);
                initHeaderWidth(tbl);
                return tbl;
            } else if (getValueAt(i, i1) instanceof TableHeaderUI) {
    ...
    private void initHeaderWidth(JTable table) {
        for (int i = 0; i < table.getColumnCount(); i++) {
            ...
            if (obj instanceof TableHeaderUI) {
                TableHeaderUI header = (TableHeaderUI) table.getValueAt(0, i);
    ...
    public TableCellEditor getCellEditor(int i, int i1) {
        return new inventory.table.TableCellEditor();
    }
